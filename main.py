# main.py - VERSÃO COMPLETA COM FORÇA RELOAD
...
def gerar_plano():
   ...
   print("=== ENDPOINT /gerarPlano CHAMADO ===")
   ...
   try:
       ...
       print(f"=== CHAMANDO generate_plan_logic COM DADOS: {request_data.get('paciente', {}).get('nome', 'Unknown')} ===")
       ...
       if status_code == 200:
           ...
           print("=== PLANO GERADO COM SUCESSO NO ENDPOINT ===")
       else:
           logger.error(f"Request {request_id} - Error generating plan: {response.get('erro', 'Unknown error')}")
           print(f"=== ERRO AO GERAR PLANO: {response.get('erro', 'Unknown error')} ===")
